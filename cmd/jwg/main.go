...
var (
	...
	transcriptTag = flag.String("transcripttag", "", "comma-separated list of transcript struct tag; if you want to transcript swagger etc tag to new JSON struct")
)
...
func main() {
	...
	{
		for _, str := range strings.Split(*transcriptTag, ",") {
			if str == "" {
				continue
			}
			transcriptTags = append(transcriptTags, str)
		}
	}

	bu, err := jwg.Parse(pInfo, typeInfos, transcriptTags)
	if err != nil {
		log.Fatal(err)
	}
